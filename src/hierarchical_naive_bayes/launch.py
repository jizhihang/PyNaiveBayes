...
def main():
    ...
    suffix += "-hierarchical_naive_bayes"
    ...
    from hierarchical_naive_bayes import monte_carlo
    bayes_mixture = monte_carlo.MonteCarlo()
    bayes_mixture._initialize(documents, number_of_clusters, alpha_alpha, alpha_beta);
    
    for iteration in xrange(number_of_iterations):
        bayes_mixture.learning();
        
        if (bayes_mixture._counter % snapshot_interval == 0):
            bayes_mixture.export_model_snapshot(output_directory, input_directory);
